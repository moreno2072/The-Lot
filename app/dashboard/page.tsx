...
export default async function DashboardPage() {
  ...
  const isApproved = store?.sellerStatus === 'APPROVED';
  const isRejected = store?.sellerStatus === 'REJECTED';
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-ink/60">
            {isApproved
              ? 'Create listings, then go live to start taking bids.'
              : 'Complete the steps below to get approved for live selling.'}
          </p>
        </div>
        {isApproved && <ConnectPayoutsButton onboarded={Boolean(store?.stripeOnboarded)} />}
      </div>

      {!isApproved && (
        <div className="mt-8 rounded border border-hairline/10 bg-white/40 px-4 py-4">
          {isRejected ? (
            <p className="font-mono text-sm text-ink/70">
              Your seller application wasn't approved. Reach out to support if you have questions.
            </p>
          ) : (
            <p className="font-mono text-sm text-ink/70">
